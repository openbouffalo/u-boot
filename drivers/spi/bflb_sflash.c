...
struct bflb_sflash_plat {
	struct bflb_sflash_regs *base;
};
			     ...
			     const struct spi_mem_op *op)
{
	...
	struct bflb_sflash_plat *plat = dev_get_plat(dev);
	struct bflb_sflash_regs *regs = plat->base;
	u32 val;
	int ret;
...
static int bflb_sflash_probe(struct udevice *dev)
{
	...
	writel(CTRL0_CLK_OUT_GATE_EN |
	       ...
	       CTRL0_READ_DLY(0) |
	       CTRL0_READ_DLY_EN |
	       CTRL0_ID(26), &plat->base->ctrl0);

	writel(CTRL1_IF_EN |
	       CTRL1_IF0_ACK_LAT(6) |
	       CTRL1_AHB2SIF_EN	|
	       CTRL1_AHB2SRAM_EN, &plat->base->ctrl1);

	return 0;
}

static int bflb_sflash_of_to_plat(struct udevice *dev)
{
	struct bflb_sflash_plat *plat = dev_get_plat(dev);
...
U_BOOT_DRIVER(bflb_sflash) = {
	...
	.of_to_plat	= bflb_sflash_of_to_plat,
	.plat_auto	= sizeof(struct bflb_sflash_plat),
	.ops		= &bflb_sflash_ops,
};
